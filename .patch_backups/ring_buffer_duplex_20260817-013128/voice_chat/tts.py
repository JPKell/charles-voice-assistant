...
class TextToSpeech:
    ...
    def speak(
        ...
        # Duplex playback releases the TTS producer slightly before a fragment
        # finishes so the next Kokoro fragment can be queued in time. Flush at
        # the end so the final fragment really reaches the speakers.
        if playback is not None and hasattr(playback, "flush"):
            return playback.flush(stop_event=stop_event)
