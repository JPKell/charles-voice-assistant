...
class DuplexBargeInSession:
    """Ring-buffered full-duplex PortAudio session."""

    COMMON_RATES = (24000, 48000, 44100, 32000, 16000)

    CALLBACK_BLOCK_SECONDS = 0.060
    OUTPUT_BUFFER_SECONDS = 4.0
    ...
    def __init__(
        ...
        *,
        ...
        self.block_frames = max(
            128,
            int(round(self.stream_rate * self.CALLBACK_BLOCK_SECONDS)),
        )
        self.output_prebuffer_frames = max(
            self.block_frames,
            int(round(self.stream_rate * self.OUTPUT_PREBUFFER_SECONDS)),
        )
        self.detector_batch_frames = max(
            self.block_frames,
            int(round(self.stream_rate * self.DETECTOR_BATCH_SECONDS)),
        )

        self._output_ring = FrameRingBuffer(
            int(round(self.stream_rate * self.OUTPUT_BUFFER_SECONDS)),
            1,
        )
        self._input_ring = FrameRingBuffer(
            int(round(self.stream_rate * self.INPUT_BUFFER_SECONDS)),
            self.cfg.channels,
        )
    ...
    def start(self) -> None:
        ...
        try:
            ...
            print(
                f"[audio] fixed duplex block unavailable ({exc}); "
                "falling back to PortAudio blocksize=0"
            )
            self._stream = sd.Stream(
                blocksize=0,
                **common,
        ...
        print(
            f"[audio] ring duplex {self.stream_rate} Hz; "
            f"block≈{1000.0 * self.block_frames / self.stream_rate:.0f}ms; "
    ...
    def _callback(self, indata, outdata, frames, _time_info, status) -> None:
        # Keep the real-time callback intentionally tiny.
        ...
        if (
            ...
            and not self.detected_event.is_set()
        ):
            wrote = self._output_ring.read_into(outdata[:, :1])
            if wrote:
                self._last_output_callback = self._callback_counter
    ...
    def play_audio(
        ...
        # Resampling happens outside the PortAudio callback.
        ...
        # Queue into the large output ring and return when buffered so Kokoro
        # can generate the next fragment immediately.
    ...
    def _playback_recently_active(self) -> bool:
        recent_callbacks = max(
            2,
            int(round(0.30 / self.CALLBACK_BLOCK_SECONDS)),
        )
        return (
            self._callback_counter - self._last_output_callback
            <= recent_callbacks
        )
    ...
    def _print_audio_stats(self) -> None:
        input_high_ms = 1000.0 * self._input_high_water_frames / self.stream_rate
        output_high_ms = 1000.0 * self._output_high_water_frames / self.stream_rate

        print(
            "[audio] ring stats: "
            f"portaudio_output_underflows={self._output_underflows}, "
