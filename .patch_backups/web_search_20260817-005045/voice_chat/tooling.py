...
Use a tool whenever the user asks about current computer, GPU, VRAM, Ollama,
or Docker state. Do not invent live system status when a tool can answer it.
Use tool results as the source of truth for current local state.

Do not claim that you changed, stopped, restarted, deleted, or modified
anything. The currently exposed tools are read-only.
""".strip()
