...
class SpeechSession:
    ...
    def add(self) -> bool:
        with self.lock:
            if self.canceled or self.closed:
                return False
            self.pending += 1
            return True

    def _claim_finish_locked(self) -> bool:
        if self.closed and self.pending == 0 and not self._finish_started:
    ...
    def _finish(self) -> None:
        try:
            if (
                ...
                and hasattr(self.playback, "flush")
            ):
                self.playback.flush(stop_event=self.stop_event)
        finally:
            self.done_event.set()
    ...
    def item_done(self) -> None:
        ...
        with self.lock:
            ...
            should_finish = self._claim_finish_locked()
